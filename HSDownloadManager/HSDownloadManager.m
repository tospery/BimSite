 ...
 *  开启任务下载资源
 */
- (void)download:(NSString *)url andFileName:(NSString*)fileName progress:(void (^)(NSInteger, NSInteger, CGFloat))progressBlock state:(void (^)(DownloadState))stateBlock
{
    if (!url) return;
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error
{
    ...
    sessionModel.stateBlock(DownloadStateCompleted);//临时会掉完成
}
